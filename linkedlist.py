...
class LinkedList:
    ...
    def delete(self, item):
        ...
        #handle case where only one item present, item is both the head and tail
        if self.head == self.tail:
            self.head = None
            self.tail = None
            return item
        
        #handle case where we delete from head
        if item == self.head.data:
            self.head = self.head.next
            return item
        
        #case where not deleting from head and more than one item in LL
        previous = self.head
